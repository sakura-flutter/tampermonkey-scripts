...
import { once, $, log, type VueHTMLElement } from '@monkey/shared/utils'
import type { Site } from '../../types'

import homeStyles from './home.scss?inline'
import playDetailStyles from './play-detail.scss?inline'
  ...
  handler() {
        ...
        proxyConfig = new Proxy(unsafeWindowAlias.$CONFIG, {
          set(target, property, value, receiver) {
            ...
            if (property === 'location' && value !== oldVal) {
              log.warn('script：reinsert styleSheet')
              addStyle()
            }
            return succeeded
          },
    ...
    const addStyle = (function () {
      ...
      return function () {
        ...
        log.warn(target, $CONFIG.location)
        if (!target) return
        uiControl.show()
