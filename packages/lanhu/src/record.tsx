...
import { qs, mountComponent } from '@monkey/shared/utils'
import { useGMvalue } from '@monkey/shared/composables'
import store from '@monkey/shared/store'
import { Button, Toast } from '@monkey/shared/components'
import { GM_registerMenuCommand, GM_getValue, GM_setValue, GM_setClipboard } from '$'
        ...
        watch(
          ...
          () => {
            nextTick(() => {
              ...
              if (first) {
                const width = [...first.children].reduce(
        ...
        >
          <Transition name="inject-slide-fade">
            <div v-show={reversed.value.length && (state.unhidden || state.recordsVisible)}>
              <TransitionGroup tag="ul" name="inject-slide-hor-fade">
                {reversed.value.map((item, index) => (
                  <li
                    ...
                    key={item.pid}
                    ref={el => {
                      el && (lisRef.value[index] = el as HTMLElement)
                    }}
                  >
                    <a href={item.href} title={item.customTitle || item.title} target="_blank">
          ...
          <div class="control">
            <Button class="view-btn" type="primary" shadow>
              打开最近项目
            </Button>
            <input v-model={state.unhidden} type="checkbox" title="固定显示" />
          </div>
        </article>
      )
    },
...
const IconCopy = (
  <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4117" width="10" height="10">
    <path
